...
define( 'DN_OMO'		, $_omokagedn = DN_DATA . '/omokage-data' );
...
//.. データ読み込み
//$dataini = _tsv_load2( DN_PREP. '/mng_config.tsv' );
define( 'MNG_CONF', _tsv_load2( DN_PREP. '/mng_config.tsv' ) );
...
$_filenames += [
	...
	//- vq (em)
	'vq30'		=> DN_OMO . '/<did>-vq30.pdb' ,
	'prevq30'	=> DN_OMO . '/<did>-prevq30.pdb' ,
	'vq50'		=> DN_OMO . '/<did>-vq50.pdb' ,
	'prevq50'	=> DN_OMO . '/<did>-prevq50.pdb' ,
	'emdb_vq30'	=> DN_OMO . '/emdb-<id>-vq30.pdb' , //- vq30と同じファイル
	'emdb_vq50'	=> DN_OMO . '/emdb-<id>-vq50.pdb' , //- vq50と同じファイル

	//- prof (em)
//	'prof30'	=> DN_OMO . '/<did>-prof30.txt' ,
//	'prof50'	=> DN_OMO . '/<did>-prof50.txt' ,
//	'profout'	=> DN_OMO . '/<did>-profout.txt' ,
//	'profpca'	=> DN_OMO . '/<did>-profpca.txt' ,
...
function _fn_taxo_json( $name ) {
	return DN_DATA. '/taxo/json/'
		. strtr( ucfirst( strtolower( $name ) ), [ ' '=>'_', ':'=>'_', '/'=>'_'  ] )
		. '.json'
	;
}
//. function 外部ツール利用系
...
//.. _blist: ブラックリスト
//- mng_config から
function _blist( $name ) {
	$ret = [];
	foreach ( MNG_CONF[ $name ] as $v )
...
class cls_blist {
	...
	function __construct( $in, $type = 'ブラックリスト' ) {
		...
		if ( ! is_array( $in ) )
			$in = explode( "\n", $in );
		foreach ( $in as $line ) {
			$line = trim( preg_replace( '/\/\/.+/', '', $line ) ); //- '//' コメント
			if ( $line == '' ) continue;
			list( $l, $r ) = explode( "\t", $line, 2 );
			$this->list[ $l ] = $r ?: true;
		}
		$this->type = $type;
		_m( "$type 読み込み - " . count( $this->list ) . '件', 'blue' );
	}
	function inc( $i ) {
		$m = $this->list[ $i ];
		$f = $m != '';
		if ( $f ) _m( "$i: " . ( $m == 1 ? $this->type : $m ), 'green' );
		return $f;
	}
}
...
class cls_pubmedid_tsv {
	...
	//.. param
	protected $data, $newdata, $db, $fn = DN_PREP . '/pubmed_id.tsv', $whitelist;
