...
//.. data
define( 'ANNOT'			, _tsv_load2( DN_EDIT. '/chem_annot.tsv' ) );
...
foreach ( _idloop( 'chem_json' ) as $pn ) {
	...
	foreach ( explode( '; ', $json->chem_comp->pdbx_synonyms ) as $s )
		_add_syn( $s, true );
	foreach ( (array)$json->pdbx_synonyms as $child ) {
		_add_syn( $child->name );
	}
	_names_form_dbs( $chem_id );
	_add_syn( $prd_name );
	...
	//.. 糖の
	$snfg_sym = '';
	foreach ( (array)$json->pdbx_identifier as $child ) {
		if ( $child->type != 'SNFG CARBOHYDRATE SYMBOL' ) continue;
		$snfg_sym = $child->identifier;
	}
...
//.. _add_syn
function _add_syn( $name, $flg_check = false ) {
	global $syn;
	$name = trim( $name );
	if ( $flg_check ) {
