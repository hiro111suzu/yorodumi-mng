	...
	foreach ( $ids as $id ) {
		...
		foreach ( $json_orig as $categ_name => $categ ) {
			if ( ! $categ[0][ 'prd_id' ] 
				...
				//- 全部に共通部
				$json_new[ $categ_name ] = $categ;
				continue;
			}
			//- 
			foreach ( $categ as $item_num => $item ) {
				if ( $item[ 'prd_id' ] != "PRD_$id" ) continue;
				$json_new[ $categ_name ][] = $item;
			}
		}
		_comp_save( $fn_out, $json_new );
	}

}
_comp_save( DN_PREP. '/prd_fam.tsv', $tsv );
//die();
